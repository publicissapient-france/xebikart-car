...
class CustomSAC(SAC):
    ...
    def plot_steps_and_reward(self, list_episode_steps, episode_reward):
        df = pd.DataFrame({'steps': list_episode_steps, 'reward': episode_reward})

        plt.figure(figsize=(15, 5))
        plt.plot(df.steps, color='red', label='Steps')
        plt.plot(df.reward, color='blue', label='Reward')
        plt.title('nombre de steps & reward')
        plt.legend()
        plt.show()
        ...
        with TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name) as writer:
            ...
            for step in range(total_timesteps):
                ...
                if done:
                    ...
                    list_episode_steps.append(ep_len)
                    self.plot_steps_and_reward(list_episode_steps, episode_rewards)
